<template>
	<view class="container">
		<view class="header">
			...
			<text class="subtitle">找回您的账号密码</text>
		</view>
		...
		<view class="content">
			<view class="card">
				...
				<view class="input-group">
					...
					<input class="input" v-model="form.newPassword" password placeholder="请输入新密码" />
				</view>
				
				<view class="input-group">
					<text class="label">确认密码</text>
					<input class="input" v-model="form.confirmPassword" password placeholder="请再次输入新密码" />
				</view>
...
export default {
	...
	methods: {
		...
		async handleReset() {
			if (!this.form.phone || !this.form.verifyCode || !this.form.newPassword || !this.form.confirmPassword) {
			...
			this.loading = true
			try {
				const res = await request.post('/resetPassword', this.form)
				if (res.code === 1 || res.code === 200) {
